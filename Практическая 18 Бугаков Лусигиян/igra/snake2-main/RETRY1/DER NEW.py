...
arg_parser.add_argument('-t', '--tiles', dest='tiles', nargs=2, metavar=('X', 'Y'), help='the number of tiles', type=int, default=[80, 90])
...
## tiles to pixels
def get_dimension(x, y, width = 5, height = 5):
    return (x * TILE_SIZE, y * TILE_SIZE, width * TILE_SIZE, height * TILE_SIZE)

## init
pygame.init()
pygame.display.set_caption('SHAKE WAR' + (' (Raspberry Mode)' if args.raspi else ''))
...
customButton = Button(110, 250, 400, 150, 'QUIT', myFunction2, True)
customButton = Button(110, 40, 400, 150, 'PLAY GAME', myFunction)
...
while winner == None:
    ...
    if food_drawn:
        ...
        if p1.x == food_x and p1.y == food_y:
            p1.tail.insert(1, (p1.x + p1.direction[1], p1.y + p1.direction[1]))
            p1.x = food_x + p1.direction[1]
            p1.y = food_y + p1.direction[1]
            p1.length += 1
            food_drawn = False
        elif p2.x == food_x and p2.y == food_y:
            p2.tail.insert(1, (p2.x + p2.direction[1], p2.y + p2.direction[1]))
            p2.x = food_x + p2.direction[1]
    ...
    if DEBUG:
        ...
        def game_over_msg():
            if winner == 0:
                if p1.length == p2.length:
                    draw = FONT_SC.render("Draw!", 1, COLOR_FG)
                    DISPLAY_SURFACE.blit(draw, (DISPLAY_SURFACE.get_width() / 2 - draw.get_rect().width / 2, 50))
                elif p1.length > p2.length:
                    game_over_msg(1)
                else:
                    game_over_msg(2)
            elif winner == 1:
                p1_wins = FONT_SC.render("Player 1 wins!", 2, COLOR_P1)
                DISPLAY_SURFACE.blit(p1_wins, (DISPLAY_SURFACE.get_width() / 1 - p1_wins.get_rect().width / 2, 200))
            elif winner == 2:
                p2_wins = FONT_SC.render("Player 2 wins!", 2, COLOR_P2)
                DISPLAY_SURFACE.blit(p2_wins, (DISPLAY_SURFACE.get_width() / 1 - p2_wins.get_rect().width / 2, 200))
...
customButton = Button(110, 250, 400, 150, 'RETRY', myFunction)
customButton = Button(110, 40, 400, 150, 'QUIT', myFunction2, True)
